	...
	for ( ; ; ) 
	{
		...
		for (int i = 0; i < nfds; ++i)
		{
			...
			if (sfd == sock_fd)//如果新监测到一个SOCKET用户连接到了绑定的SOCKET端口，建立新的连接。
			{
				...
				//设置用于读操作的文件描述符
				ret = epoll_manager.EpollCtlAdd(conn_fd, EPOLLIN);
				if (ret < 0)
				{
			...
			else if (events[i].events & EPOLLIN)//如果是已经连接的用户，并且收到数据，那么进行读入。
			{
				...
				if (bReadOk == true)
				{
					...
					ret = epoll_manager.EpollCtlMod(sfd, EPOLLOUT);
					if (ret < 0)
					{
			...
			else if (events[i].events & EPOLLOUT) // 如果有数据发送
			{
				...
				if (bWritten == true)
				{
					DEBUG_FLOWLOG("EPOLLOUT bWritten = true");
					ret = epoll_manager.EpollCtlMod(sfd, EPOLLIN);
					if (ret < 0)
					{
